...
const ProductDetail = () => {
    ...
    const deleteItem = async(id) =>{
        ...
        try {
            ...
            toast.success("Product delete successfully !!!",{position:"top-right"});
    ...
    return (
        <div>
            <div className="py-5 flex justify-between items-center">
                {/* text  */}
                <h1 className=" text-xl text-pink-500 font-bold">All Product</h1>
                {/* Add Product Button  */}
                <Link to={'/addproduct'}>
                    <button className="px-5 py-2 bg-pink-200 border border-pink-100 rounded-lg">Add Product</button>
                </Link>
            </div>
                        ...
                        {getAllProduct.map((item, index) => {
                            ...
                            return (
                                <tr key={index} className="text-pink-300">
                                    ...
                                    Delete</td> */}
                                    <td><Link to={`/updateproduct/${id}`}><button className='h-10 px-6 flex rounded-md bg-green-600 duration-300 hover:bg-green-700 text-white p-2'>Edit</button></Link></td>
                                    <td><button onClick={()=> deleteItem(id)} className='h-10 px-6 flex rounded-md bg-red-600 duration-300 hover:bg-red-700 text-white p-2'>Delete</button></td>
                                </tr>
                            )
                        })}
                    </tbody>
